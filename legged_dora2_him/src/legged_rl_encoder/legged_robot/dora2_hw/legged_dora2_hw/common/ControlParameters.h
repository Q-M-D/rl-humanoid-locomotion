...
class ControlParameter {
 public:
  ...
  ControlParameter(const std::string& name, Vec4<double>& value,
                   ControlParameterCollection& collection,
                   const std::string& units = "") {
    _name = name;
    truncateName();
    _units = units;
    _value.vec4d = value.data();
    _kind = ControlParameterValueKind::VEC4_DOUBLE;
    collection.addParameter(this, name);
  }
  ControlParameter(const std::string& name, Vec5<double>& value,
                   ControlParameterCollection& collection,
                   const std::string& units = "") {
    _name = name;
    truncateName();
    _units = units;
    _value.vec5d = value.data();
    _kind = ControlParameterValueKind::VEC5_DOUBLE;
    collection.addParameter(this, name);
  }
  ...
  void initializeVec4d(const Vec4<double>& v) {
    if (_kind != ControlParameterValueKind::VEC4_DOUBLE) {
      throw std::runtime_error("Tried to initialize control parameter " +
                               _name + " as a vector3d");
    }
    _set = true;
  ...
  void initializeVec5d(const Vec5<double>& v) {
    if (_kind != ControlParameterValueKind::VEC5_DOUBLE) {
      throw std::runtime_error("Tried to initialize control parameter " +
                               _name + " as a vector3d");
    }
    _set = true;
