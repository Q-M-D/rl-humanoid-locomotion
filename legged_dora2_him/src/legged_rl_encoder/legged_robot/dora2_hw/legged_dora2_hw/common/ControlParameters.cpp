...
void ControlParameters::initializeFromYamlFile(const std::string& path) {
  ...
  for (auto& key : keys) {
    ...
    switch (cp._kind) {
      ...
      case ControlParameterValueKind::VEC4_DOUBLE: {
        std::vector<double> vv;
        paramHandler.getVector(key, vv);
        Vec4<double> v(vv[0], vv[1], vv[2], vv[3]);
        cp.initializeVec4d(v);
      } break;

      case ControlParameterValueKind::VEC5_DOUBLE: {
        std::vector<double> vv;
        paramHandler.getVector(key, vv);
        Vec5<double> v;
        v << vv[0], vv[1], vv[2], vv[3], vv[4];
        cp.initializeVec5d(v);
      } break;
